...
对于前者来讲，目前的主流的解决方案都是 `LFS`, 主流的托管平台都支持。但在 GitOps 的大前提下，这个问题仍然算是未解决，因为在企业场景下又多了一个 Git 大文件管理的 `dashboard`， 而 GitOps 得了理念是为了减少 dashboard, 将所有的一切都统一起来。 一个可能的选项是用[Cloud Native Artifact Registries](https://hangyan.github.io/post/2019-08-26-cnar/)的理念，用[distribution](https://github.com/docker/distribution) 作为 LFS Server。
